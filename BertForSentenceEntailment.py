from preprocess import SentenceEntailment
...
class BertForSentenceEntailment(nn.Module):
    def __init__(self, config, pretrained_model_dir=None):
        super(BertForSentenceEntailment, self).__init__()
...
def train(model_config, dataset_name):
    # 处理数据
    data_loader = SentenceEntailment.LoadPairSentenceClassificationDataset(
    ...
    # 读取模型
    model = BertForSentenceEntailment(model_config,
                                      model_config.pretrained_model_dir)
    if not os.path.exists(os.path.join(model_config.pretrained_model_dir, 'BertForSentenceEntailment')):
        os.makedirs(os.path.join(model_config.pretrained_model_dir, 'BertForSentenceEntailment'))
    model_save_path = os.path.join(model_config.pretrained_model_dir, 'BertForSentenceEntailment',
                                   dataset_name + '_pt')
    ...
    for epoch in range(model_config.epochs):
        ...
        for idx, (data, seg, label) in enumerate(train_iter):
            ...
            loss, logits = model(input_ids=data, attention_mask=padding_mask, token_type_id=seg, position_ids=None,
                                 labels=label)
            losses += loss.item()
            optimizer.zero_grad()
            loss.backward()
...
if __name__ == '__main__':
    ...
    train(model_config, dataset_name='multinli')
